...
boost::asio::serial_port sp(io_ser);
...
union receiveData
{
	short d;
	unsigned char data[2];
}leftVelNow,rightVelNow,angleNow,pos_x,pos_y;
...
出口参数：
********************************************************/
void serialInit(string port_name)
{
    sp.open(port_name);
...
出口参数：bool
********************************************************/
bool readSpeed(double &Left_v,double &Right_v,double &Angle,double &Pos_X,double &Pos_Y, unsigned char &ctrlFlag)
{
    char i, length = 0;
    ...
    // 检查信息校验值
    checkSum = getCrc8(buf, 3 + length);             //buf[14] 计算得出
    if (checkSum != buf[3 + length])                 //buf[14] 串口接收
    {
    ...
    for(i = 0; i < 2; i++)
    {
        ...
        pos_x.data[i]       = buf[i + 9]; //buf[9] buf[10]
        pos_y.data[i]       = buf[i + 11];//buf[11] buf[12]
    }
    ...
    Angle   = double(angleNow.d);
    Pos_X   = double(pos_x.d);
    Pos_Y   = double(pos_y.d); 
